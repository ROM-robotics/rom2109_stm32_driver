...
int32_t getMotorA_rpm()
{
	...
	// -----------------------ROS2 CONTROL---------------------------- //
	global_A_count += (A_count_diff * (direction_bit));
	// --------------------END ROS2 CONTROL--------------------------- // 
...
int32_t getMotorB_rpm()
{
	...
	// -----------------------ROS2 CONTROL---------------------------- //
	global_B_count += (B_count_diff * (direction_bit));
	// --------------------END ROS2 CONTROL--------------------------- // 
...
int32_t getMotorD_rpm()
{
	...
	int16_t D_count_diff=0;
	int16_t actual_rpm=0;
	int8_t direction_bit = -1;
	
	if( TIM5->CR1 & TIM_CR1_DIR )													// if reverse
	{
		D_count_diff = D_prev_count - D_count;
		if( D_count_diff < 0 ) { D_count_diff += 65536; }
		direction_bit = 1;
	}
	else
	{
		D_count_diff = D_count - D_prev_count;
		if( D_count_diff < 0 ) { D_count_diff += 65536; }  	// forward counter overflow
	}
	// -----------------------ROS2 CONTROL---------------------------- //
	global_D_count += (D_count_diff * (-direction_bit));
	// --------------------END ROS2 CONTROL--------------------------- // 
